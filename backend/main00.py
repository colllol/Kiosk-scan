...
async def kill_chrome():
    """Kill Chrome và mở lại ở chế độ kiosk."""
    try:
        import subprocess
        subprocess.Popen(["python", os.path.join(SCRIPT_DIR, "killchrome.py")],
                         creationflags=subprocess.CREATE_NO_WINDOW)
        print("[KILL-CHROME] ✅ Đã gửi lệnh kill Chrome")
        return {"success": True, "message": "Đã gửi lệnh kill Chrome"}
    except Exception as e:
        print(f"[KILL-CHROME] Error: {e}")
        raise HTTPException(status_code=500, detail=f"Lỗi kill Chrome: {str(e)}")
