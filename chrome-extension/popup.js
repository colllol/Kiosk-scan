...
(function() {
  ...
  // Update page status with service info
  function updatePageStatus(isValid, url, serviceInfo) {
    if (!pageStatus) return;
    
    if (isValid) {
      let serviceText = '';
      if (serviceInfo) {
        if (serviceInfo.isSignatureCertification) {
          serviceText = '📝 <strong>Dịch vụ: Chứng thực chữ ký</strong>';
        } else if (serviceInfo.isDocumentCertification) {
          serviceText = '📄 <strong>Dịch vụ: Chứng thực tài liệu</strong>';
        } else {
          serviceText = `📋 <strong>Dịch vụ: ${serviceInfo.description || 'Không xác định'}</strong>`;
        }
      }
      
      pageStatus.innerHTML = `
        <div style="color: #155724; font-weight: 600;">✅ Trang hợp lệ</div>
        <div style="font-size: 10px; color: #666; margin-top: 4px;">${url}</div>
        ${serviceText ? `<div style="margin-top: 8px; padding: 6px; background: #e8f5e9; border-radius: 4px; font-size: 12px;">${serviceText}</div>` : ''}
      `;
      pageStatus.className = 'page-status valid';
    } else {
      pageStatus.innerHTML = `
        <div style="color: #721c24; font-weight: 600;">❌ Trang không hợp lệ</div>
        <div style="font-size: 10px; color: #666; margin-top: 4px;">${url}</div>
        <div style="margin-top: 8px; font-size: 11px; color: #666;">Extension chỉ hoạt động trên trang dịch vụ công Thái Nguyên</div>
      `;
  ...
  function checkCurrentPage() {
    ...
    pageStatus.textContent = 'Đang kiểm tra trang...';
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      ...
      // Gửi message để content script kiểm tra trang
      chrome.tabs.sendMessage(tab.id, { action: 'checkPage' }, (response) => {
        if (chrome.runtime.lastError) {
          // Content script chưa được inject, fallback về kiểm tra URL
          const isValid = VALID_URL_PATTERNS.some(pattern => pattern.test(tab.url));
          updatePageStatus(isValid, tab.url, null);
        } else {
          updatePageStatus(response.isValid, response.url, response.serviceInfo);
        }
      });
    });
  }
