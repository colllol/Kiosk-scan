...
async function processTicketForUrl(url, tabId) {
  try {
    ...
    try {
      ...
      if (queueResponse.ok) {
        queueSuccess = true;
        console.log('[BG] ✅ QueueSystem API success (text/plain)');
      } else {
        // Try with no Content-Type header
        console.log('[BG] ❌ QueueSystem API error with text/plain, trying without Content-Type...');
        try {
          queueResponse = await fetch(QUEUESYSTEM_API, {
            method: 'POST',
            body: JSON.stringify(queuePayload)
          ...
          if (queueResponse.ok) {
            queueSuccess = true;
            console.log('[BG] ✅ QueueSystem API success (no Content-Type)');
          } else {
            queueErrorText = await queueResponse.text();
            console.error('[BG] ❌ QueueSystem API error:', queueResponse.status, queueErrorText);
          }
        } catch (secondError) {
          console.error('[BG] ❌ QueueSystem fetch failed (second attempt):', secondError.name, secondError.message);
          queueSuccess = false;
        }
      }
    } catch (fetchError) {
      console.error('[BG] ❌ QueueSystem fetch failed:', fetchError.name, fetchError.message);
      console.error('[BG] 💡 Kiểm tra: Extension đã reload chưa? Server 27.71.20.120 có bật không?');
